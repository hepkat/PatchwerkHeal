...
--heal frame
	...
	if not PatchwerkHealFrame then
		...
		PatchwerkHealFrame:SetAttribute("type", "macro") -- left click causes macro
		...
		PatchwerkHealFrame:SetScript("OnReceiveDrag",function()
			if InCombatLockdown() then
				print(L["CannotModifySpellInCombat"])
				ClearCursor()
				return
			end
			...
			if ret[1]=="spell" then
				...
				--DPrint(playername,guid,classId,spellID,rank)
				if guid and spellName then
					local type, zero, server_id, instance_id, zone_uid, npc_id, spawn_uid = strsplit("-",guid);
					if type=="Player" then
						--DPrint("/cast "..'[@"' .. playername .. '",help,nodead] '..spellName..rank)
						PatchwerkHealFrame.playername=playername
						PatchwerkHealFrame.playerid=guid		

						PatchwerkHealFrame:SetAttribute("macrotext1", "/cast ".."[@" .. playername .. ",help,nodead] "..spellName..rank)
						PatchwerkHealFrame.TipText=("%s%s|r\n%s"):format(make_class_prefix(classId),PatchwerkHealFrame.playername,spellName..rank)
					...
					PatchwerkHealFrame:SetAttribute("macro",'/script print("'..L["HelpTips"]..'")') 
					PatchwerkHealFrame.HPTip:SetText(L["Not Setting"])
...
-- enable heal on click
function PatchwerkHeal:stopHeal()
	PatchwerkHealMask.maskbg:SetVertexColor(0.8,0.1,0.1,0.6)
	PatchwerkHealMask:EnableMouse(true) 
end
	
-- disable heal on click
function PatchwerkHeal:startHeal()
...
function PatchwerkHeal:CombatLogEvent(timestamp, eventtype, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags, ...)
	...
	if PatchwerkHealFrame.playerid~=nil and PatchwerkHealFrame.playerid==dstGUID then
		...
		--enable for 1s after hateful
		if eventtype=="SPELL_DAMAGE" then
			local data={...}
			if (data[2]=="Hateful Strike" or data[2] == "Frostbolt") then
				self:startHeal()
